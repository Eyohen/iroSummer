...
const Login = () => {
  ...
  return (
    ...
    <div className=' flex items-center justify-center h-[100vh]'>
        ...
        <input onChange={(e) => setEmail(e.target.value)} className='border border-[#D7D7D7] w-full md:w-[400px] py-2 px-3 rounded-lg hover:border-red-500' />
    ...
    <input onChange={(e) => setPassword(e.target.value)} className="border rounded-lg w-full py-2 px-3 leading-tight hover:border-red-500 pr-16 font-mono " type={isPasswordVisible ? "text" : "password"} autocomplete="off"
    />
  ...
  <div className='flex justify-between'>
    ...
    <p className='text-red-500'>Forgot Password</p>
        ...
        <button onClick={handleSubmit}  className='bg-[#F7F7F7] text-[#98999A] w-full md:w-[400px] py-2 rounded-2xl mt-6 hover:bg-red-300 hover:text-white'>{isLoading ? "Loading..." : "Login"}</button>
        {error && <h3 className='text-red-500 text-lg text-center'>Something went wrong</h3>}
        </div>
        <p className='pt-3 text-center text-[#98999A]'>Don't have an account?   <Link to={'/register'}><span className='text-red-500 ml-1'>Create an account</span></Link></p>
        </div>
